...
[[***clarkevans.test( )***](https://www.rdocumentation.org/packages/spatstat/versions/1.63-3/topics/clarkevans.test) - statspat -]{style="color:#d46e15"} to perform Clark-Evans test of aggregation for a spatial point pattern.

-   `correction` argument has 4 options of edge correction - `none`, `Donnelly` , `guard` and `cdf`. Other none and all, the rest aims to provide a more accurate estimate of the mean nearest neighbour distance by accounting for the potential bias introduced by points near the edges of the study region.
:::
...
Not applicable. Donnelly correction is only available for rectangular windows.

## GUARD

Not applicable??

Error: Can't interpret W as a window.
